...
{
    public string RootPath { get; set; } = @"C:\kwi-automations\projects\DuplicateMerging";
    ...
    public DuplicateMergeConfig DuplicateMerge { get; set; } = new();
    ...
    public string PendingReviewFolder => Path.Combine(RootPath, DuplicateMerge.PendingReviewFolder);
    public string ApprovedFolder => Path.Combine(RootPath, DuplicateMerge.ApprovedFolder);
    public string CompletedFolder => Path.Combine(RootPath, DuplicateMerge.CompletedFolder);
    public string FailedFolder => Path.Combine(RootPath, DuplicateMerge.FailedFolder);
    public string HistoryFolder => Path.Combine(RootPath, DuplicateMerge.HistoryFolder);
    public string ControlFolder => Path.Combine(RootPath, DuplicateMerge.ControlFolder);
...
public sealed class SegmentationConfig
{
    ...
    public string PlanWorkbookPrefix { get; set; } = "account_segmentation_plan";
    public string PlannedParentHistoryFile { get; set; } = "planned_parent_accounts.txt";
...
public sealed class BatchLimitConfig
{
    ...
    public int MaxConcurrentApplyRows { get; set; } = 10;
    public int ApplyWorkbookSaveEveryRows { get; set; } = 100;
}

public sealed class DuplicateMergeConfig
{
    public string PlanWorkbookPrefix { get; set; } = "duplicate_merge_plan";
    public int MaxAccountsToScanPerPlan { get; set; } = 500;
    public int PlanBatchSize { get; set; } = 1000;
    public int MaxConcurrentScanAccounts { get; set; } = 3;
    public string AccountCodeScanStart { get; set; } = "00000000";
    public string AccountCodeScanEnd { get; set; } = "99999999";
    public bool RequireCountryMatch { get; set; } = true;
    public bool IncludeOnlyActiveAndProspectiveAccounts { get; set; } = true;
    public string ControlFolder { get; set; } = "00 Control";
    public string PendingReviewFolder { get; set; } = "01 Pending Review";
    public string ApprovedFolder { get; set; } = "02 Approved";
    public string CompletedFolder { get; set; } = "03 Completed";
    public string FailedFolder { get; set; } = "04 Failed";
    public string HistoryFolder { get; set; } = "99 History";
    public string ScanCheckpointFile { get; set; } = "scan_checkpoint.json";
    public string PlannedGroupHistoryFile { get; set; } = "planned_duplicate_groups.tsv";
    public string ProcessedPlanHistoryFile { get; set; } = "processed_plan_history.tsv";
    public string FailedScanHistoryFile { get; set; } = "failed_duplicate_scan_items.tsv";
}

public sealed class ScanCheckpoint
{
    public string NextAccountCode { get; set; } = "00000000";
