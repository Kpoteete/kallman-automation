...
public sealed class MomentusSdkApiService : IMomentusApiService
{
    ...
    public async Task<IReadOnlyList<MomentusAccountSnapshot>> SearchActiveParentAccountsAsync(int maxResults, CancellationToken cancellationToken)
    {
        return await _retryPolicy.ExecuteAsync(async () =>
        {
            ...
            var accounts = await SearchActiveParentAccountsInCodeSlicesAsync(safeMax, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<IReadOnlyList<MomentusAuditAccountSnapshot>> SearchAuditAccountsByClassAsync(string classValue, int maxResults, CancellationToken cancellationToken)
    {
        return await _retryPolicy.ExecuteAsync(async () =>
        {
            string cleanClass = TextUtil.CleanKeyField(classValue);
            if (string.IsNullOrWhiteSpace(cleanClass)) return new List<MomentusAuditAccountSnapshot>();

            int safeMax = Math.Max(1, maxResults);
            List<object> accounts = await SearchAccountsByClassInCodeSlicesAsync(cleanClass, safeMax, cancellationToken)
                .ConfigureAwait(false);

            return accounts
                .Select(ToAuditAccountSnapshot)
                .Where(a => !string.IsNullOrWhiteSpace(a.AccountCode))
                .OrderBy(a => a.AccountCode, StringComparer.OrdinalIgnoreCase)
                .Take(safeMax)
                .ToList();
    ...
    public async Task<MomentusAccountSnapshot?> GetAccountByCodeAsync(string accountCode, CancellationToken cancellationToken)
    {
        return await _retryPolicy.ExecuteAsync(async () =>
        {
            string cleanAccountCode = TextUtil.CleanKeyField(accountCode);
            if (string.IsNullOrWhiteSpace(cleanAccountCode)) return null;

            string odata = $"{_fields.AccountCode} eq '{ODataString(cleanAccountCode)}'";
            List<object> accounts = await SearchAccountsAsync(odata, cancellationToken, maxResults: 10)
                .ConfigureAwait(false);

            object? match = accounts.FirstOrDefault(account =>
                TextUtil.EqualsTrimmedIgnoreCase(GetString(account, _fields.AccountCode, "AccountCode", "Code"), cleanAccountCode));

            return match is null ? null : ToAccountSnapshot(match);
        }, cancellationToken).ConfigureAwait(false);
    }

    private async Task<List<object>> SearchActiveParentAccountsInCodeSlicesAsync(int maxResults, CancellationToken cancellationToken)
    {
        const int apiSafeSliceSize = 1000;
        const int width = 8;
        long scanStart = ParseAccountCode(_config.Segmentation.AccountCodeScanStart, 0);
        long scanEnd = ParseAccountCode(_config.Segmentation.AccountCodeScanEnd, 99999999);
        if (scanEnd < scanStart) scanEnd = scanStart;

        var byAccountCode = new Dictionary<string, object>(StringComparer.OrdinalIgnoreCase);

        for (long sliceStart = scanStart; sliceStart <= scanEnd && byAccountCode.Count < maxResults; sliceStart += apiSafeSliceSize)
        {
            cancellationToken.ThrowIfCancellationRequested();

            long sliceEnd = Math.Min(scanEnd, sliceStart + apiSafeSliceSize - 1);
    ...
    private async Task<List<object>> SearchAccountsByClassInCodeSlicesAsync(string classValue, int maxResults, CancellationToken cancellationToken)
    {
        const int apiSafeSliceSize = 1000;
        const int width = 8;
        long scanStart = ParseAccountCode(_config.Segmentation.AccountCodeScanStart, 0);
        ...
        for (long sliceStart = scanStart; sliceStart <= scanEnd && byAccountCode.Count < maxResults; sliceStart += apiSafeSliceSize)
        {
            ...
            string odata =
                $"{_fields.Class} eq '{ODataString(classValue)}' and " +
                $"{_fields.AccountCode} ge '{ODataString(sliceStartCode)}' and " +
                $"{_fields.AccountCode} le '{ODataString(sliceEndCode)}'";
    ...
    private static long ParseAccountCode(string? accountCode, long fallback)
    {
        string digits = new string((accountCode ?? string.Empty).Where(char.IsDigit).ToArray());
        if (string.IsNullOrWhiteSpace(digits)) return fallback;
        return long.TryParse(digits, out long value) ? Math.Max(0, value) : fallback;
    }
    ...
    private MomentusAccountSnapshot ToAccountSnapshot(object model)
    {
        return new MomentusAccountSnapshot(
            ...
            GetString(model, _fields.Class, "Class", "AccountClass", "ClassCode"),
            GetString(model, "AccountStatus", "Status"),
    ...
    private MomentusAuditAccountSnapshot ToAuditAccountSnapshot(object model)
    {
        return new MomentusAuditAccountSnapshot(
            GetString(model, _fields.AccountCode, "AccountCode", "Code"),
            GetString(model, _fields.AccountName, "Name", "AccountName", "CompanyName"),
            GetString(model, _fields.Class, "Class", "AccountClass", "ClassCode"),
            GetString(model, "AccountStatus", "Status"),
